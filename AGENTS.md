...
## Project Overview
- Treease 是一个多格式结构化文档工具链：`packages/core/` 负责 Rust 解析/格式化/算子/评估/建图，`apps/web/` 负责编辑器与图形界面，`apps/cli/` 保留 CLI acceptance 测试与文档入口。
...
- CLI 主链：`packages/core/src/tools/treease.rs` → `treease_core::cli::main()`
...
- CLI 常用：`cd packages/core && cargo nextest run --locked --lib cli::`；`cd apps/cli && bash tests/acceptance/run.sh`
