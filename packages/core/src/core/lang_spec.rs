...
pub fn parse_tree(language_name: &str, source: &[u8]) -> Option<tree_sitter::Tree> {
    let language = crate::core::tree_sitter_support::tree_sitter_language(language_name)?;
    let mut parser = tree_sitter::Parser::new();
    parser.set_language(&language).ok()?;
    let syntax_source =
        crate::core::tree_sitter_support::tree_sitter_syntax_source(language_name, source);
