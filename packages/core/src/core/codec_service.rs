...
use super::{
    CoreError, FormatError, FormatLanguage, FormatRegistry, Language, NodeId, ParseError,
    TreeStore, format_from_string, tree_sitter_support,
};
...
impl CodecService {
    ...
    pub fn get_encoder(
        ...
        if let Some(registry) = &self.registry {
            let registry_prefs = super::format_registry::FormatPreferences {
                indent,
                pretty: indent != 0,
            };
    ...
    pub fn get_decoder(&self, format_name: &str) -> Result<Box<dyn Decode>, CoreError> {
        if let Some(registry) = &self.registry {
            let registry_prefs = super::format_registry::FormatPreferences::default();
            let canonical = canonical_format_name(format_name)?;
            return registry
