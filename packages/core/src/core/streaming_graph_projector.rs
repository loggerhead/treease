...
use crate::core::graph_topology::{self, GraphTopology, SequencePresentationState};
    ...
    pub fn take_incremental_state(&mut self) -> Option<IncrementalState> {
        ...
        Some(
            IncrementalState::resumable()
                .with_graph_state(
                    crate::core::graph_builder::GraphModelSnapshot::owned(model),
                    index,
                )
...
mod tests {
    use super::*;
    use crate::core::layout_engine::LayoutMetrics;
    ...
    fn streaming_scrollable_headerless_seq_rows_do_not_publish_child_nodes_after_close() {
        ...
        let items_node = final_model
            ...
            .find(|node| {
                node.path.len() == 1
                    && matches!(node.path.first(), Some(crate::core::graph_builder::PathSeg::Key(key)) if key == "items")
            })
    ...
    fn streaming_root_mapping_children_match_baseline_layout_after_close() {
        ...
        let baseline_model = {
            let b = builder_from_source(&source);
            let (s, r) = b.tree_ref().unwrap();
            crate::core::graph_projection_service::build_graph_model_for_tree(s, r, "json")
                .expect("baseline model build")
        };
        // path -> (y, height, table_rows) for every baseline node.
        let model_by_path = |m: &crate::core::graph_builder::GraphModel| {
            m.nodes
                .iter()
                .map(|n| {
                    let path = n
                        ...
                        .iter()
                        .map(|seg| match seg {
                            crate::core::graph_builder::PathSeg::Key(k) => k.clone(),
                            crate::core::graph_builder::PathSeg::Index(i) => format!("[{i}]"),
                        })
                        .collect::<Vec<_>>()
                        .join(".");
                    (
    ...
    fn baseline_header_table_nested_cells_stay_folded_in_main_graph() {
        ...
        let model =
            crate::core::graph_projection_service::build_graph_model_for_tree(store, root, "json")
                .expect("baseline model build");
    ...
    fn materializer_appends_batch_with_dense_handles() {
        ...
        let materialized =
            crate::core::graph_materialize::append_batch_into_current_model(&mut model, batch, &[]);
