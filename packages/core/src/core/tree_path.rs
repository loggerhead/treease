...
/// using a pre-built [`crate::core::TreePathIndex`] to avoid linear scanning.
///
...
pub fn find_node_by_path_with_index(
    ...
    prefer_key: bool,
    store: &TreeStore,
    index: Option<&crate::core::TreePathIndex>,
) -> Option<NodeId> {
...
pub struct PathSpanResolver<'a> {
    ...
    path_index_ref: Option<&'a crate::core::TreePathIndex>,
}
...
impl<'a> PathSpanResolver<'a> {
    ...
    pub fn with_tree_path_index(mut self, index: &'a crate::core::TreePathIndex) -> Self {
            ...
            && resolved.column <= 0
        {
            ...
            if let Some(segments) = path {
                ...
                path_buffer = segments
                    .iter()
                    .map(|seg| match seg {
                        crate::core::OwnedPathSeg::Key(key) => path_seg_key_owned(key.clone()),
                        crate::core::OwnedPathSeg::Index(index) => path_seg_index(*index),
                    })
                    .collect();
            };
...
/// [`crate::core::TreePathIndex`] to avoid building a fresh index or scanning
/// siblings linearly.
