...
pub struct IncrementalGraphDeltaResult {
    pub model_snapshot: crate::core::GraphModelSnapshot,
...
fn build_impacted_direct_delta_candidate(
    ...
    for node in &model.nodes {
        ...
        let new_hash = crate::core::graph_model_index::hash_graph_node(node);
...
fn build_table_scalar_cell_delta(
    ...
    if table_cell_patch_preserves_geometry(
        ...
        let model_snapshot = crate::core::GraphModelSnapshot::owned(old_model.clone())
    ...
    Some(IncrementalGraphDeltaResult {
        model_snapshot: crate::core::GraphModelSnapshot::owned(merged_model),
        graph_index: merged_graph_index,
        delta,
        relayout_chain: Vec::new(),
        table_cell_patches: Vec::new(),
...
fn is_leaf(node: &super::TreeNode) -> bool {
