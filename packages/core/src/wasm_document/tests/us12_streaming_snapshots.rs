...
#[test]
fn wasm_document_escape_nest_json_roundtrip_1mb_fixture() {
    let _guard = lock_test_mutex();
    reset_test_state();
    ...
    // with nest mode enabled. The nest parser will recursively expand the
    // escaped content back into the original structure.
    ...
    assert_eq!(
        ...
        "escape → nest-parse roundtrip should produce identical graph"
    );
}
