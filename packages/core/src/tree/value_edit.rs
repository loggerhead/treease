...
fn encode_document(language: &str, store: &TreeStore, root: NodeId) -> Result<String, CoreError> {
...
fn clone_subtree(
    ...
    let src = src_store
        .get(src_id)
        .ok_or_else(|| ValueEditError::Core(CoreError::Io("source node missing".into())))?;
    let mut cloned = src.clone();
    cloned.parent = parent;
    cloned.is_map_key = false;
    cloned.set_sequence_index(None);
    cloned.value = crate::tree::NodeValueRef::Missing;
    ...
    for child_id in old_content {
        ...
        dst_store
            .get_mut(new_id)
            .ok_or_else(|| ValueEditError::Core(CoreError::Io("node missing after add".into())))?
            .content
            .push(child_new);
    }
    Ok(new_id)
...
fn overwrite_node(
    ...
    let src = src_store
        ...
        .ok_or_else(|| ValueEditError::Core(CoreError::Io("source node missing".into())))?;
    {
        let target_node = dst_store
            .get_mut(target)
            .ok_or_else(|| ValueEditError::Core(CoreError::Io("target node missing".into())))?;
        target_node.kind = src.kind;
        target_node.sem_type = src.sem_type;
        target_node.tag = src.tag.clone();
        target_node.value = crate::tree::NodeValueRef::Missing;
        ...
        .content
    {
        ...
        dst_store
            .get_mut(target)
            .ok_or_else(|| {
                ValueEditError::Core(CoreError::Io("node missing after overwrite".into()))
            })?
            .content
            .push(child_new);
    }
