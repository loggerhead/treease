...
use crate::core::lang_spec::{GraphValueEditRuleKind, find_spec};
...
pub(crate) fn plan_graph_value_edit(
    ...
    path_index: Option<&crate::core::TreePathIndex>,
) -> GraphValueEditPlan {
...
pub(super) fn request_path_segments(path: &[GraphPathSeg]) -> Vec<PathSeg<'_>> {
    path.iter()
        .map(|segment| {
            if segment.tag == 1 {
                crate::core::path_seg_index(segment.index)
            } else {
                crate::core::path_seg_key(&segment.key)
            }
        })
        .collect()
}
