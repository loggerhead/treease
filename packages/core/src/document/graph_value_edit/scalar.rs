use crate::core::{DocumentTextEdit, TreeNode, TreeNodeKind};
...
impl<R: ScalarGraphValueEditRules> GraphValueEditPlanner for ScalarGraphValueEditPlanner<R> {
    fn plan(&self, ctx: GraphValueEditContext<'_>) -> GraphValueEditPlan {
        ...
        let target = crate::core::find_node_by_path_with_index(
            ctx.document.root,
            &path,
...
impl<R: ScalarGraphValueEditRules> ScalarGraphValueEditPlanner<R> {
    ...
    fn resolve_span(
        ...
        let span = crate::core::compute_path_span_for_document_with_index(
            &ctx.document.store,
            ctx.document.root,
