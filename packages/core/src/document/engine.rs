...
mod tests {
    ...
    /// Large streaming fixture: close path performance regression test.
    /// The 188KB graph-table-missing-row fixture is fed as many 16KB chunks (≈ the test
    /// close this must complete well under 10s.
    #[test]
    fn streaming_json_large_fixture_small_chunks_close_completes() {
        let source = include_str!("../../../../test/fixtures/json/graph-table-missing-row.1.json");
        let chunk_size = crate::stream::chunk_size::select_chunk_size(source.as_bytes().len());

        let mut rt = DocumentRuntime::default();
        let mut m = DocumentEngineMetrics::default();
        let handle = add_streaming_job(&mut rt, "stream-large-close");
        ...
        for chunk in source.as_bytes().chunks(chunk_size) {
            ...
            let batch = advance_job(
                ...
                &mut m,
                handle,
                AdvanceInput::TextChunk(text.into()),
            );
            assert!(batch.terminal.is_none(), "chunk should not terminal");
        }

        let start = std::time::Instant::now();
        let batch = advance_job(&mut rt, &mut m, handle, AdvanceInput::Close);
        let elapsed = start.elapsed();

        assert!(
            matches!(batch.terminal, Some(JobTerminal::Completed)),
            "close should complete, terminal={:?} elapsed={:?}",
            batch.terminal,
            elapsed,
        );
        assert!(
    ...
    /// chunks, close.  Must complete under 10s (the E2E test budget).
    #[test]
    fn streaming_json_large_inline_500kb_close_completes() {
        const TARGET_BYTES: usize = 500 * 1024;
        const RECORD: &str = r#"{"name":"test","value":12345},"#;
        let chunk_size = crate::stream::chunk_size::select_chunk_size(TARGET_BYTES);
        ...
        let mut rt = DocumentRuntime::default();
        let mut m = DocumentEngineMetrics::default();
        let handle = add_streaming_job(&mut rt, "stream-500kb");
        ...
        assert!(
            matches!(batch.terminal, Some(JobTerminal::Completed)),
            "close should succeed, terminal={:?} elapsed={:?}",
            batch.terminal,
            elapsed,
        );
        assert!(
            elapsed.as_secs() < 10,
            "streaming close of 500KB fixture took {:.1}s — exceeds 10s budget",
            elapsed.as_secs_f64(),
        );
    }

    /// 2MB fixture stress test — matches the E2E 2MB hover test scenario.
    #[test]
    fn streaming_json_2mb_close_completes() {
        ...
        let mut m = DocumentEngineMetrics::default();
        let h = rt.allocate_job_handle();
        let seq = rt.allocate_request_seq("stream-2mb");
        rt.jobs.insert(
            h,
            JobEntry {
                spec: DocumentJobSpec {
                    kind: DocumentJobKind::AnalyzeSource,
                    document_key: "stream-2mb".to_owned(),
                    language: "json".to_owned(),
                    input: DocumentInputPlan::SourceText,
                    settings: crate::document::protocol::DocumentJobSettings::default(),
                    output: OutputPlan {
        ...
        for chunk in source.as_bytes().chunks(chunk_size) {
            let text = std::str::from_utf8(chunk).expect("chunk is valid UTF-8");
            let batch = advance_job(&mut rt, &mut m, h, AdvanceInput::TextChunk(text.into()));
            assert!(batch.terminal.is_none(), "chunk should not terminal");
        }
        let streaming_elapsed = streaming_start.elapsed();
        let streaming_budget = if std::env::var("CI").is_ok() { 60 } else { 15 };
        assert!(
            streaming_elapsed.as_secs() < streaming_budget,
            "streaming chunks of 2MB fixture took {:.1}s — exceeds {streaming_budget}s budget",
            streaming_elapsed.as_secs_f64(),
        );

        let start = std::time::Instant::now();
        let _batch = advance_job(&mut rt, &mut m, h, AdvanceInput::Close);
        let elapsed = start.elapsed();
        let close_budget = if std::env::var("CI").is_ok() { 30 } else { 15 };
        assert!(
            elapsed.as_secs() < close_budget,
            "streaming close of 2MB fixture took {:.1}s — exceeds {close_budget}s budget",
            elapsed.as_secs_f64(),
        );
    }

    /// Real 2MB fixture (from E2E tests), fed as 16KB chunks.
    #[test]
    fn streaming_json_fixture_2mb_close_completes() {
        let source = include_str!("../../../../test/fixtures/json/2mb.1.json");
        let chunk_size = crate::stream::chunk_size::select_chunk_size(source.as_bytes().len());

        let mut rt = DocumentRuntime::default();
        let mut m = DocumentEngineMetrics::default();
        let handle = add_streaming_job(&mut rt, "stream-fixture-2mb");

        let mut offset = 0;
        let bytes = source.as_bytes();
        while offset < bytes.len() {
            let end = std::cmp::min(offset + chunk_size, bytes.len());
            // Find the last valid UTF-8 char boundary within this chunk.
            let chunk_end = if end < bytes.len() {
                let mut p = end;
                while p > offset && bytes[p] & 0xC0 == 0x80 {
                    p -= 1;
                }
                p
            } else {
            ...
            let text =
                std::str::from_utf8(&bytes[offset..chunk_end]).expect("chunk is valid UTF-8");
            let batch = advance_job(
                &mut rt,
                &mut m,
                handle,
        ...
        assert!(
            elapsed.as_secs() < 30,
            "streaming close of 2mb.json fixture took {:.1}s — exceeds 30s budget",
            elapsed.as_secs_f64(),
        );
    }
