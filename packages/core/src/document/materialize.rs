use crate::{
    core::{
        CoreError, NodeId, SemType, StructuralSpanIndex, TokenSpan, TreeNode, TreeNodeKind,
        TreeStore, adjust_tree_store_offsets_from_collecting, find_exact_node_for_edit,
        ...
        parse_scalar_edit_replacement, recompute_tree_store_locations_for,
    },
    formats::DecodedDocument,
    stream::{self, DecodeOptions},
};

use crate::core::document_analysis::encode_document_value_json;
use crate::core::line_index::LineIndex;

use super::input::ByteStream;
use super::protocol::{DocumentInputPlan, JobTerminal, OutputPlan};
use super::runtime::store_snapshot_for_document;
use super::snapshot::{
...
fn try_structural_materialize(
    ...
    let delta = crate::core::edit_delta(edit);
...
fn semantic_tokens_for_structural_edit(
    ...
    match (
        ...
        _ => crate::core::encode_semantic_tokens(language, source_text),
    }
}
...
fn build_incremental_graph_output(
    ...
    base_incremental: Option<&IncrementalState>,
    old_model: &crate::core::graph_builder::GraphModel,
    ...
    let old_index =
        if let Some(index) = base_incremental.and_then(|state| state.graph_model_index.as_ref()) {
            ...
            old_index_owned = crate::core::GraphModelIndex::build(old_model);
            &old_index_owned
        };
    let builder_config =
    ...
    let result = crate::core::graph_delta_service::build_incremental_graph_delta_with_index(
        old_model,
        old_index,
        &decoded.store,
        decoded.root,
...
fn build_resumed_graph_output(
    ...
    let mut projector =
        crate::core::streaming_graph_projector::StreamingGraphProjector::from_incremental_state(
            language,
            document_key,
            base,
        )?;
...
fn apply_subtree_node_edit(
    ...
    let new_boundary_end = apply_delta(old_boundary.end_byte, crate::core::edit_delta(edit))?;
...
fn attach_tree_path_index(
    ...
    incremental.map(|state| {
        state.with_tree_path_index(crate::core::TreePathIndex::build(
            &decoded.store,
            decoded.root,
        ))
    })
...
fn structural_tree_path_index(
    ...
    if let Some(base_index) = base_incremental.and_then(|state| state.tree_path_index.as_ref()) {
        return base_index.updated_for_structural_edit(
            &decoded.store,
            crate::core::TreePathIndexStructuralUpdate {
...
mod tests {
    ...
    fn input_edit_positions_use_byte_columns() {
        ...
        let input_edit = crate::core::input_edit_for_source(source, &edit)
    ...
    fn input_edit_rejects_invalid_byte_boundaries() {
        ...
        assert!(crate::core::input_edit_for_source(source, &edit).is_none());
    }
    ...
    fn materialize_structural_reencodes_semantic_tokens_for_type_change() {
        ...
        assert_eq!(
            updated.analysis.semantic_tokens,
            crate::core::encode_semantic_tokens("json", &updated.analysis.source)
        );
    }
    ...
    fn materialize_structural_subtree_edit_refreshes_tree_path_index_for_changed_subtree() {
        ...
        let c_id = crate::core::find_node_by_path_with_index(
