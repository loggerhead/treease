...
- `SnapshotReady { snapshotId, analysis, mainGraph }` 是主文档权威终态；`analysis` 只携带轻量 diagnostics / semantic tokens / metadata，不默认下发 full tree 或 full valueJson；`AnalysisDelta` 只表达流式过程中的临时可见结果。
- snapshot-bound read API 统一返回 `SnapshotReadResult<T>`；缺少 snapshot 时返回 `SnapshotNotReady`，不得偷偷创建 snapshot。
- `query_snapshot` 承载 snapshot-bound 局部 projection，例如 root value kind、node preview、path scalar value 与 field labels；不要新增默认 full tree/full value 读取路径。
