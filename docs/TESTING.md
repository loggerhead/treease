...
- 需要度量性能时，统一使用 `packages/core/src/test_timing.rs`，不要临时散落新的计时入口。
- 调整或补充性能观测时，优先扩展 `test_timing.rs`，保持性能口径集中、可复用、可对比。
- 清理无用代码、测试或临时文件时，不要删除 `packages/core/src/test_timing.rs`；它是仓库内约定的性能度量入口，不按“暂时未被调用”处理。
