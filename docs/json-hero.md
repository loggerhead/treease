...
read_when:
  - The task involves the JSON Hero-style column navigator.
  - The task involves column navigation, leaf detail editing, or Column Navigator keyboard navigation.
---
...
`column navigator` 仅表示历史生命周期和数据域术语；列轨道、路径导航和详情编辑器
均属于 Column Navigator。

## Goal

Replace the current column navigator's column rail with the `Column Navigator`, a path-driven column browser modeled on JSON Hero. The algorithm and interaction model should follow JSON Hero's Column View, while the visual language, theme, typography, spacing, and components remain Treease-native.

The main Treease graph and editor remain unchanged. This document defines the bottom column navigator only.
...
- Selection, history, and keyboard navigation within the Column Navigator.
...
The Column Navigator is a projection of one active tree path. Each column represents one object or array on that path and displays its direct children.
...
the Column Navigator renders a corresponding sequence of columns from left to right.
...
- Columns are not independent Column Rails and do not maintain an unrelated column path.
- The Column Navigator must preserve the full path chain; it must not truncate or compress the chain to a fixed number of visible columns.
...
If the next selected node is a leaf, or there is no next container to render, the Column Navigator does not create another column. It opens the right-side detail editor instead.
...
When the selected node is a non-empty object or array, the Column Navigator keeps its
...
The `Column Detail Editor` uses Monaco Editor and follows the current column navigator editing contract:

- It uses the same Monaco-based Column Detail Editor logic as the existing column navigator.
...
- It must handle commit failures, stale asynchronous results, and refreshes using the existing column navigator behavior.
...
- The Column Navigator uses one native horizontal scrolling container.
...
- Mouse-wheel input follows native browser scrolling and must not be converted into Column Navigator pan or zoom.
...
The graph canvas pan/zoom model is not used for the column rail. The Column Navigator is a data browser; it is not a collection of independently movable graph canvases.
...
- Back and Forward navigate Column Navigator history.
...
Keyboard navigation is enabled only while focus is inside the column navigator.

- The Column Navigator root must be focusable.
- Focusing any interactive element inside the Column Navigator counts as Column Navigator focus.
- Keyboard handlers must be scoped to the Column Navigator focus boundary rather than installed as global document/window handlers.
- Arrow keys, `Esc`, history shortcuts, and copy/navigation shortcuts must not intercept input while focus is in the main editor, Monaco detail editor, search controls, top bar, or any other area outside the Column Navigator.
- When Monaco has text-editing focus, normal Monaco cursor and selection behavior takes precedence over Column Navigator navigation. Column Navigator-level navigation may resume after focus returns to the Column Navigator navigation surface.
- Moving focus outside the Column Navigator immediately disables Column Navigator keyboard navigation.

## Treease Integration

The replacement changes the column navigator presentation and navigation model, not Treease's document authority:

- Core parsing, paths, snapshots, and projections remain the source of data.
- The main graph remains the primary graph view.
- The Column Navigator remains a persistent bottom Column Navigator, not a hover preview or a second document.
- Treease's existing bidirectional-edit semantics remain authoritative for the right-side Monaco editor.
- Column Navigator state must continue to refresh or invalidate when the active snapshot, revision, render configuration, or nesting configuration changes.
- Closing or removing the Column Navigator releases temporary editor and projection state.
- Whole-document replacement, import, language switch, and initial-example reconstruction reset the Column Navigator according to the existing lifecycle contract.

## State Requirements

The Column Navigator state must have one canonical active path and derive the following from it:
...
- A container opens as a JSON Hero-style column path, not as a Leafer Column Rail.
...
- Column Navigator keyboard navigation works when focus is inside the Column Navigator.
- The same keyboard shortcuts do not affect the main editor or other UI when focus is outside the Column Navigator.
...
The current implementation lives in the Web GraphViewer Column Navigator controller
...
The right-side detail editor is the existing Monaco Column Detail Editor surface. It
...
Column Navigator keyboard handling is scoped to the Column Navigator focus boundary. Arrow
...
The feature is covered by Column Navigator controller/unit tests, DOM Column Navigator tests,
and Column Navigator integration coverage. The relevant local proof is:
...
pnpm build:e2e
pnpm exec playwright test test/e2e/column-navigator.spec.ts test/e2e/column-navigator-edit-click-highlight-regression.spec.ts --workers=1
pnpm check:circular
```
