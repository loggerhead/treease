...
See more regular expression examples under the [`string` operator docs](string-operators.md).
...
```bash
treease '.[] | select(test("[a-zA-Z]+_[0-9]$"))' sample.yml
```
...
```bash
treease '.[] | select(. == "cat" or test("og$"))' sample.yml
```
...
```bash
treease 'with_entries(select(.key | test("ame$")))' sample.yml
```
...
```bash
treease '(.a.[] | select(. == "cat" or . == "goat")) |= "rabbit"' sample.yml
```
