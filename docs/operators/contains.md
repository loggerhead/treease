...
then
```bash
treease 'contains(["baz", "bar"])' sample.yml
```
will output
```yaml
true
...
then
```bash
treease '["baz", "bar"] - . | length == 0' sample.yml
```
will output
```yaml
false
...
then
```bash
treease 'contains({"bar": [{"barp": 12}]})' sample.yml
```
will output
```yaml
true
...
then
```bash
treease 'contains({"foo": 12, "bar": [{"barp": 15}]})' sample.yml
```
will output
```yaml
false
...
then
```bash
treease 'contains("bar")' sample.yml
```
will output
```yaml
true
...
then
```bash
treease 'contains("meow")' sample.yml
```
will output
```yaml
true
