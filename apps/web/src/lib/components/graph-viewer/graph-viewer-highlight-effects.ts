...
export function buildGraphHighlightSignature(
  ...
  return `${graphHighlight.revision}|${graphHighlight.target ?? 'auto'}|${graphHighlight.navigate === true ? 'navigate' : 'highlight'}|${graphHighlight.revealToken ?? ''}|${buildPathKey(graphHighlight.path)}`;
}
