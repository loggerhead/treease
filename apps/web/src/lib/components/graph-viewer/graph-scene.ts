...
type GraphSceneControllerDeps = {
  ...
  updateCellEntry: (map: Map<string, any>, cell: any, updater: (entry: any) => void) => void;
  upsertClickTargetProbe: (store: GraphViewerClickTargetStore, targetIds: WeakMap<object, string>, box: any, cell: any, kind: any) => string;
  ...
  resolveInteractiveCellPath: (cell: any, fallbackPath: any[]) => Promise<any[]>;
  emitReveal: (path: any[], target: 'key' | 'value' | 'node', source: 'click' | 'test-hook') => void;
