...
const mockStreamUpdateHandler = vi.hoisted(() => ({
  ...
  applyGraphDeltaToState: vi.fn((delta: any, state: any) => {
    ...
    for (const patch of delta.layoutPatches ?? []) {
      const nodeId =
        patch.render_handle ?? patch.renderHandle ?? patch.group_handle ?? patch.groupHandle;
...
describe("graph-scene-runtime", () => {
  ...
  it("keeps the full graph while materializing only the viewport projection and pinned search target", () => {
    const { runtime, nodeBoxMap } = createDeps({ viewport: { width: 400, height: 300 } });
    ...
    expect(runtime.ensureNodeMaterialized(101)).toBe(true);
