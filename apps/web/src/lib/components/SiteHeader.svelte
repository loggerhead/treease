...
<style>
  ...
  .brand-mark { font-family: var(--font-display, "Avenir Next", sans-serif); font-size: 18px; font-weight: 700; letter-spacing: -0.03em; }
  .brand-note { color: var(--muted-soft, #718196); font-size: 13px; letter-spacing: 0.02em; white-space: nowrap; }
  .site-nav { display: inline-flex; align-items: center; gap: 18px; }
  .site-nav a { color: var(--muted); font-size: 14px; font-weight: 600; text-decoration: none; transition: color 160ms ease; }
  .site-nav a:hover { color: var(--accent-strong); }
  .header-cta { display: inline-flex; align-items: center; justify-content: center; min-height: 48px; padding: 0 22px; border-radius: 999px; font-size: 15px; font-weight: 700; text-decoration: none; transition: transform 140ms ease, border-color 160ms ease, background-color 160ms ease, color 160ms ease, box-shadow 160ms ease; color: #fff; background: linear-gradient(135deg, var(--accent) 0%, var(--accent-strong) 100%); box-shadow: 0 16px 34px rgba(23, 69, 181, 0.22); }
  .header-cta:hover { transform: translateY(-1px); }
  .header-actions { display: inline-flex; align-items: center; gap: 14px; }
  @media (max-width: 900px) {
    .site-header { grid-template-columns: 1fr; gap: 16px; justify-items: start; }
