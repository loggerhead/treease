...
describe('EditorDiagnostics', () => {
  ...
  it('stores analysis eagerly for the current document', async () => {
    const callMock = vi.mocked(callSharedWasmWorker as any);
    callMock
      .mockResolvedValueOnce({ jobHandle: 1, batch: { requestSeq: 1, events: [], terminal: null } })
      .mockResolvedValueOnce({ requestSeq: 1, events: [], terminal: null })
      .mockResolvedValueOnce({
        requestSeq: 1,
        events: [{ type: 'snapshotReady', snapshotId: 7, analysis: null, mainGraph: null }],
        terminal: { type: 'completed' },
      });

    await analyzeDocumentAndStore('json' as any, '{"a":1}', 'vitest://diag/store', true);
  ...
  it('keeps editor analysis jobs in the original source coordinate space', async () => {
    const callMock = vi.mocked(callSharedWasmWorker as any);
    callMock
      .mockResolvedValueOnce({ jobHandle: 1, batch: { requestSeq: 1, events: [], terminal: null } })
      .mockResolvedValueOnce({ requestSeq: 1, events: [], terminal: null })
    ...
    await analyzeDocumentAndStore('json' as any, '{"a":{"b":1}}', 'vitest://diag/source-coords', true);

    expect(callMock.mock.calls[0][0]).toBe('startDocumentJob');
  ...
  it('forwards streaming analysis deltas before the terminal snapshot', async () => {
    const callMock = vi.mocked(callSharedWasmWorker as any);
    const onAnalysisDelta = vi.fn();
    callMock
      .mockResolvedValueOnce({ jobHandle: 1, batch: { requestSeq: 1, events: [], terminal: null } })
      .mockResolvedValueOnce({
        requestSeq: 1,
        events: [
          {
            ...
            analysis: {
              ...
              semanticTokens: { data: [0, 1, 3, 0, 0], version: 1 },
              sourceByteLength: 7,
              language: 'json',
            },
          },
    ...
    const result = await analyzeDocumentAndStore('json' as any, '{"a":1}', 'vitest://diag/streaming', true, {
      onAnalysisDelta,
    });
    ...
    expect(Array.from(new Uint32Array(result!.semanticTokens!))).toEqual([0, 1, 3, 0, 0]);
    expect(onAnalysisDelta).toHaveBeenCalledWith(
      expect.objectContaining({
        diagnostics: [{ startLineNumber: 1, startColumn: 1, endLineNumber: 1, endColumn: 2, kind: 1 }],
        snapshotId: null,
      }),
