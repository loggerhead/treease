...
describe('share restore', () => {
  it('restores compare text, applies compare, then restores both viewport anchors', async () => {
    ...
    }, { ...ports(calls), setViewMode: (mode) => calls.push(`mode.${mode}`), clearCompareState: () => calls.push('compare.clear'), restoreTreePath: () => true, restoreGraphFocus: async () => true, waitForGraphReady: async () => true, rebuildSubgraphWorkspace: async () => true, reportNavigationWarning: () => calls.push('warning') });
  ...
  it('reports graph focus restoration when the runtime cannot resolve the target', async () => {
    ...
    await restoreShareResource({
      type: 'text_snapshot',
      payload: { schemaVersion: 1, left: { text: '{"left":1}', languageId: 'json' }, right: null, layout: { viewMode: 'graph', activePane: 'left' }, interaction: { treePath: [], focus: { type: 'graph', path: [{ type: 'key', key: 'missing' }], target: 'node', editorSelection: { startLine: 1, startColumn: 1, endLine: 1, endColumn: 1 } }, subgraphWorkspace: { panePaths: [] } } },
    }, { ...ports(calls), setViewMode: (mode) => calls.push(`mode.${mode}`), clearCompareState: () => calls.push('compare.clear'), restoreTreePath: () => true, restoreGraphFocus: async () => false, waitForGraphReady: async () => true, rebuildSubgraphWorkspace: async () => true, reportNavigationWarning: () => calls.push('warning') });
