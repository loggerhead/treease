...
describe('share restore', () => {
  it('restores compare text, applies compare, then restores both viewport anchors', async () => {
    ...
    }, { ...ports(calls), setViewMode: (mode) => calls.push(`mode.${mode}`), clearCompareState: () => calls.push('compare.clear'), restoreTreePath: () => true, restoreGraphFocus: () => true, waitForGraphReady: async () => true, rebuildSubgraphWorkspace: async () => true, reportNavigationWarning: () => calls.push('warning') });
  ...
  it('restores the source selection before graph focus', async () => {
    ...
    }, { ...ports(calls), setViewMode: (mode) => calls.push(`mode.${mode}`), clearCompareState: () => calls.push('compare.clear'), restoreTreePath: () => true, restoreGraphFocus: () => { calls.push('graph.focus'); return true; }, waitForGraphReady: async () => { calls.push('graph.ready'); return true; }, rebuildSubgraphWorkspace: async () => true, reportNavigationWarning: () => calls.push('warning') });
