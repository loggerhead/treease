...
function collectHunks(result: RawDiffResult): FlatDiff[] {
  ...
  for (const pair of result.pairs ?? []) {
    if (pair.hasLeft && pair.left) {
      hunks.push({ offset: pair.left.offset, length: pair.left.length, type: pair.left.type });
    }
    if (pair.hasRight && pair.right) {
      hunks.push({ offset: pair.right.offset, length: pair.right.length, type: pair.right.type });
    }
  }
