...
describe("renderGraphEdges", () => {
  ...
  it("draws bezier curves with configured edge color", () => {
    ...
    expect(PenCtor.pens[0].setStyle).toHaveBeenCalledWith({
      stroke: "#ccc",
      strokeWidth: 1,
    });
    expect(PenCtor.pens[0].moveTo).toHaveBeenCalledWith(100, 25);
    expect(PenCtor.pens[0].bezierCurveTo).toHaveBeenCalledWith(
      110,
      25,
      120,
      25,
  ...
  it("draws an edge from its Bezier geometry when endpoints are not materialized", () => {
    const PenCtor = createMockPenCtor();
    const layer = { removeAll: vi.fn(), add: vi.fn() };
    renderGraphEdges({
    ...
    expect(PenCtor.pens).toHaveLength(1);
    expect(layer.add).toHaveBeenCalledWith(PenCtor.pens[0]);
  });
});
