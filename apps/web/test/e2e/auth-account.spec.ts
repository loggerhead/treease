...
test('PKCE login returns to the original page, shows the account, and supports logout', async ({ page }) => {
  ...
  await expect(page.getByTestId('account-avatar-button')).toHaveCSS('cursor', 'pointer');
  await page.getByTestId('account-avatar-button').click();
  await expect(page.getByTestId('account-details')).toContainText('Ada Lovelace');
  await expect(page.getByTestId('account-details')).toContainText('ada@example.com');
  await expect(page.getByTestId('account-check-updates-menu-item')).toHaveCount(0);
